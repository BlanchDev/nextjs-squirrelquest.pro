...
export async function GET(request) {
  try {
    ...
    if (!isValidSession) {
      ...
      const isLimited = await isRateLimited(ip, "admin_sessions_unauthorized");
      if (isLimited) {
        return NextResponse.json(
          {
            success: false,
      ...
      await logRequest(ip, "admin_sessions_unauthorized");
    ...
    const [sessions] = await pool.query(
      "SELECT * FROM adminsessions WHERE isDeleted = 0 ORDER BY id DESC",
    );

    // Log the successful access
    // await logRequest(ip, "admin_sessions_view");
